...
Интеграционные тесты автоматически поднимают NATS через `docker compose` (с уникальным проектом и портом) и in-process SMTP-сервер `aiosmtpd`. Временные файлы создаются в `/tmp/atlas_email/` и не удаляются после прогона — для отладки.
